...
volatile tim1_t *tim1_addr;
volatile gpio_t *gpioe_addr;
...
static int __init motor_init(void)
{
	...
	// 完成物理地址到虚拟地址的映射
	gpioe_addr = ioremap(GPIOE_A, sizeof(gpio_t));
	if (gpioe_addr == NULL) {
        printk("phy addr to virl addr failed\n");
        return -ENOMEM;
    }

	tim1_addr = ioremap(TIM1_A, sizeof(tim1_t));
	if (tim1_addr == NULL) {
        printk("phy addr to virl addr failed\n");
        return -ENOMEM;
    }
	...
	/*******rcc_addr章节********/
	//1.使能GPIOB时钟 MP_AHB4ENSETR[1] = 1
	rcc_addr->MP_AHB4ENSETR |=(0x1<<4);
	//2.使能TIM4时钟 MP_APB1ENSETR[2] = 1
	rcc_addr->MP_APB2ENSETR |=(0x1<<0);
	
	/*********GPIO章节************/
	//1.通过MODER寄存器，设置PB6为复用模式 MODER[13:12] = 10
	gpioe_addr->MODER &=(~(0x3<<18));
	gpioe_addr->MODER |=((0x2<<18));
	//2.设置AFRL寄存器，设置PB6引脚复用功能为TIM4_CH1 AFRL[27:24] = 0010
	gpioe_addr->AFRH &=(~(0xf<<4));
	gpioe_addr->AFRH |=(0x1<<4);
	
	/*********TIM4章节**********/
	//1.设置分频器，将时钟源进行分频 PSC[15:0] = 209 -1  *
	tim1_addr->PSC &=(~(0xffff));
	tim1_addr->PSC |=(0xd0);
	//2.设置自动重载计数器的值 PWM方波的频率1000HZ ARR[15:0] = 1000 *
	tim1_addr->ARR &=(~(0xffff));
	tim1_addr->ARR |=((0x3e8));
	//3.设置捕获/比较寄存器中的值，PWM方波占空比 CCR1[15:0] = 700  *
	tim1_addr->CCR1 &=(~(0xffff));
	tim1_addr->CCR1 |=((0x320));
	tim1_addr->CCR1 |=((0x1f4));
	//4.设置TIM4_CH1通道为PWM1模式 CCMR1[16] [6:4] = 0110 *
	tim1_addr->CCMR1 &=(~(0x1<<16));
	tim1_addr->CCMR1 &=(~(0x7<<4));
	tim1_addr->CCMR1 |=((0x6<<4));
	//5.设置TIM4_CH1通道预加载使能 CCMR1[3] = 1 *
	tim1_addr->CCMR1 |=(0x1<<3);
	//6.设置TIM4_CH1通道捕获/比较寄存器输出模式 CCMR1[1:0] = 00 *
	tim1_addr->CCMR1 &=(~(0x3));
	//7.设置TIM4_CH1通道配置为输出极性模式 CCER[3] = 0 *
	tim1_addr->CCER &=(~(0x1<<3));
	//8.设置TIM4_CH1通道起始状态为低/高电平 CCER[1] = 0 *
	tim1_addr->CCER &=(~(0x1<<1));
